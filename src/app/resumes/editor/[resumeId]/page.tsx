...
import {
  ...
  type ResumeImprovementSuggestionsInput,
  type ResumeImprovementSuggestionsOutput,
} from "@/ai/flows/resume-improvement-suggestions";
...
export default function ResumeEditorPage() {
  ...
  const handleSaveResume = async () => {
    ...
    toast({
      title: "Resume Saved!",
      description: `Changes to ${activeResume.versionName} have been saved.`,
    });
  };
  ...
  const handleGetAtsSuggestions = async () => {
    ...
    try {
      ...
      if (activeResume.contact.linkedin)
        resumeFullContent += `LinkedIn: ${activeResume.contact.linkedin}\n`;
      resumeFullContent += `\nSummary:\n${activeResume.summary}\n`;

      if (activeResume.experience.length > 0) {
        resumeFullContent += `\nExperience:\n`;
        activeResume.experience.forEach((exp) => {
          resumeFullContent += `- ${exp.jobTitle} at ${exp.company} (${
            exp.startDate
      ...
      if (activeResume.customSections.length > 0) {
        activeResume.customSections.forEach((section) => {
          ...
          section.items.forEach((item) => {
            resumeFullContent += `- ${item.content}${
      ...
      toast({
        ...
        description: `Your new ATS score is ${suggestionsResult.atsScore}%.`,
      });
    } catch (error) {
      console.error("Resume ATS suggestion failed:", error);
      toast({
        title: "Error",
        description: "Failed to analyze resume for ATS.",
        variant: "destructive",
      });
    } finally {
      ...
      <AppShell>
        <div className="flex flex-col items-center justify-center h-full text-center">
          ...
          <p className="text-muted-foreground">
            The resume you are looking for does not exist or could not be
            loaded.
          </p>
          <Button onClick={() => router.push("/resumes")} className="mt-4">
            Go to Dashboard
          </Button>
        </div>
      </AppShell>
  ...
  const templateOptions = [
    {
      id: "classic",
      name: "Classic",
      imageUrl: "https://placehold.co/150x200.png?text=Classic+Design",
    ...
    },
    {
      id: "modern",
      name: "Modern",
      imageUrl: "https://placehold.co/150x200.png?text=Modern+Design",
      description: "A sleek, contemporary style.",
      dataAiHint: "modern resume",
    },
    {
      id: "compact",
      name: "Compact",
      imageUrl: "https://placehold.co/150x200.png?text=Compact+Design",
      description: "A space-saving, concise format.",
      dataAiHint: "compact resume",
    },
    {
      id: "two-column-classic",
      name: "Two Column",
      imageUrl: "https://placehold.co/150x200.png?text=Two+Column",
      description: "Classic two-column structure.",
      dataAiHint: "two column resume",
    },
    {
      id: "creative",
      name: "Creative",
      imageUrl: "https://placehold.co/150x200.png?text=Creative",
      description: "A bold, stylish design.",
                ...
                <TabsContent value="design">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Select a style that best fits your profile.
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        <div className="flex items-center justify-between rounded-lg border p-3">
                          <div className="space-y-0.5">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Display your photo on the resume. (Recommended for
                              Creative templates)
                            </p>
                          </div>
                          <Switch
                            ...
                            onCheckedChange={(checked) =>
                              handleUpdateField("settings.showPhoto", checked)
                            }
                      ...
                      <p className="text-muted-foreground text-sm mt-6">
                        More customization options like font and color coming
                        soon!
                      </p>
                    </CardContent>
                  </Card>
                ...
                <TabsContent value="ats">
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Analyze your resume against a job description to improve
                        your chances.
                      </CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      ...
                      <Button
                        onClick={handleGetAtsSuggestions}
                        disabled={isLoadingAITailoring}
                      >
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
                                ...
                                {prevAtsScore !== null && (
                                  <div className="text-center p-4 border-l">
                                    ...
                                    <p className="text-4xl font-bold text-muted-foreground line-through">
                                      {prevAtsScore}%
                                    </p>
                                    <p
                                      className={cn(
                                        "mt-2 text-lg font-semibold",
                                        atsReport.atsScore > prevAtsScore
                                          ? "text-green-500"
                                          : "text-destructive"
                                      )}
                                    >
                                      Change:{" "}
                                      {atsReport.atsScore - prevAtsScore > 0
                                        ? "+"
                                        : ""}
                                      {atsReport.atsScore - prevAtsScore}%
                                    </p>
